module CASClient
  module Integration
    class Filter
      ...
      class << self
        attr_reader :config, :client, :fake_user, :fake_extra_attribues
        ...
        def filter(controller)
          ...
          if st && last_st &&
            ...
            controller.logger.warn("Re-using previously validated ticket since the ticket id and service are the same.")
            return true
          elsif last_st &&
            ...
            controller.logger.debug "Existing local CAS session detected for #{controller.session[@client.username_session_key].inspect}. "+
          ...
          if st
            ...
            if st.is_valid?
              #if is_new_session
              controller.logger.info("Ticket #{st.ticket.inspect} for service #{st.service.inspect} belonging to user #{st.user.inspect} is VALID.")
              controller.session[@client.username_session_key] = st.user.dup
              controller.session[@client.extra_attributes_session_key] = Hash.new(st.extra_attributes) if st.extra_attributes

              if st.extra_attributes
                controller.logger.debug("Extra user attributes provided along with ticket #{st.ticket.inspect}: #{st.extra_attributes.inspect}.")
              end
              ...
              if st.pgt_iou
                unless controller.session[:cas_pgt] && controller.session[:cas_pgt].ticket && controller.session[:cas_pgt].iou == st.pgt_iou
                  controller.logger.info("Receipt has a proxy-granting ticket IOU. Attempting to retrieve the proxy-granting ticket...")
                  pgt = @client.retrieve_proxy_granting_ticket(st.pgt_iou)

                  if pgt
                    controller.logger.debug("Got PGT #{pgt.ticket.inspect} for PGT IOU #{pgt.iou.inspect}. This will be stored in the session.")
                    controller.session[:cas_pgt] = pgt
                    # For backwards compatibility with RubyCAS-Client 1.x configurations...
                    controller.session[:casfilterpgt] = pgt
                  else
                    controller.logger.error("Failed to retrieve a PGT for PGT IOU #{st.pgt_iou}!")
                  end
                else
                  controller.logger.info("PGT is present in session and PGT IOU #{st.pgt_iou} matches the saved PGT IOU.  Not retrieving new PGT.")
                end
              end
              return true
            else
              controller.logger.warn("Ticket #{st.ticket.inspect} failed validation -- #{st.failure_code}: #{st.failure_message}")
              unauthorized!(controller, st)
              return false
            end
          else # no service ticket was present in the request
            if returning_from_gateway?(controller)
              controller.logger.info "Returning from CAS gateway without authentication."

              # unset, to allow for the next request to be authenticated if necessary
              controller.session[:cas_sent_to_gateway] = false

              if use_gatewaying?
                controller.logger.info "This CAS client is configured to use gatewaying, so we will permit the user to continue without authentication."
                controller.session[@client.username_session_key] = nil
                return true
              else
                controller.logger.warn "The CAS client is NOT configured to allow gatewaying, yet this request was gatewayed. Something is not right!"
              end
            end
          ...
          controller.logger.error("SSL Error: hostname was not match with the server certificate. You can try to disable the ssl verification with a :force_ssl_verification => false in your configurations file.")
          unauthorized!(controller)
          return false
        end

        def configure(config)
          raise ArgumentError, "Missing :adapter on Filter configuration." unless config[:adapter]
        ...
        def login_url(controller)
          ...
          controller.logger.debug("Generated login url: #{url}")
          return url
        end
        ...
        def login_to_service(controller, credentials, return_path)
          ...
          if resp.is_failure?
            controller.logger.info("Validation failed for service #{return_path.inspect} reason: '#{resp.failure_message}'")
          else
            controller.logger.info("Ticket #{resp.ticket.inspect} for service #{return_path.inspect} is VALID.")
          end
        ...
        def logout(controller, service = nil)
          ...
          @config[:adapter].reset_session(controller)
          @config[:adapter].redirect(controller, @client.logout_url(referer))
        end

        def unauthorized!(controller, vr = nil)
          #
          # TODO: refractor and decouple Rails methods
          #
          format = (controller.request.respond_to?(:format)) ? controller.request.format.to_sym : :html
        ...
        def redirect_to_cas_for_authentication(controller)
          ...
          if controller.session[:previous_redirect_to_cas] &&
            ...
            controller.logger.warn("Previous redirect to the CAS server was less than a second ago. The client at #{controller.request.remote_ip.inspect} may be stuck in a redirection loop!")
            controller.session[:cas_validation_retry_count] ||= 0

            if controller.session[:cas_validation_retry_count] > 3
              controller.logger.error("Redirection loop intercepted. Client at #{controller.request.remote_ip.inspect} will be redirected back to login page and forced to renew authentication.")
          ...
          controller.session[:cas_redirect] = redirect_url
          @config[:adapter].redirect(controller, redirect_url)
        end
        ...
        def single_sign_out(controller)
          ...
          headers = controller.request.respond_to?(:headers) ? controller.request.headers : controller.request
          ...
          if controller.request.post? &&
            ...
            [controller.params['logoutRequest'],URI.unescape(controller.params['logoutRequest'])].find{|xml| xml =~
                                                                                                       ...
                                                                                                       unless config[:enable_single_sign_out]
                                                                                                         controller.logger.warn "Ignoring single-sign-out request for CAS session #{si.inspect} because ssout functionality is not enabled (see the :enable_single_sign_out config option)."
                                                                                                         return false
                                                                                                       end

                                                                                                       controller.logger.debug "Intercepted single-sign-out request for CAS session #{si.inspect}."
        ...
        def read_ticket(controller)
          ...
          controller.logger.debug("Request contains ticket #{ticket.inspect}.")
        ...
        def read_service_url(controller)
          if @config[:service_url]
            controller.logger.debug("Using explicitly set service url: #{@config[:service_url]}")
            return @config[:service_url]
          end

          service_url = @config[:adapter].service_url(controller)
          controller.logger.debug("Guessed service url: #{service_url.inspect}")
          return service_url
        end
      end
    end
